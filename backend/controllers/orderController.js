...
export const updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    if (status === 'validated' && order.patient_id) {
      notifications.push({
    ...
    } else if (status === 'rejected' && order.patient_id) {
      notifications.push({
        userId: order.patient_id,
        notification: {
    ...
    } else if (status === 'preparing' && order.patient_id) {
      notifications.push({
        userId: order.patient_id,
        notification: {
    ...
    } else if (status === 'ready' && order.patient_id) {
      notifications.push({
        userId: order.patient_id,
        notification: {
      ...
      if (driverId) {
        notifications.push({
          ...
          notification: {
            ...
            message: `Vous avez été assigné à la livraison #${orderId.slice(-6)}`,
            type: 'info',
            orderId: orderId
          }
        });
      }
    } else if (status === 'in_transit' && order.patient_id) {
      notifications.push({
        userId: order.patient_id,
        notification: {
      ...
      if (order.pharmacy_owner_id) {
        notifications.push({
          userId: order.pharmacy_owner_id,
          notification: {
            title: 'Livraison en cours 🚚',
            message: `La commande #${orderId.slice(-6)} est en cours de livraison`,
            type: 'info',
            orderId: orderId
          }
        });
      }
    } else if (status === 'delivered' && order.patient_id) {
      notifications.push({
        userId: order.patient_id,
        notification: {
      ...
      if (order.pharmacy_owner_id) {
        notifications.push({
          ...
          notification: {
            title: 'Commande livrée ✅',
            message: `La commande #${orderId.slice(-6)} a été livrée avec succès`,
            type: 'success',
            orderId: orderId
          }
        });
